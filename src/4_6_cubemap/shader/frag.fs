...
{   
    ...
    //反射
    float reflectIntensity = texture(texture_reflect1, uv).r;
    vec3 reflectDir = reflect(-viewDir, normal);
    vec3 reflection = texture(environmentMapping, reflectDir).rgb * reflectIntensity;
    
    FragColor = vec4(finalColor + reflection, 1.0f);
}